...
void ControllerDevice::SetDirection(float x, float y, float rx, float ry, float a, float b)
{
    if(x == 0.0f && y == 0.0f)
...
vr::EVRInitError ControllerDevice::Activate(uint32_t unObjectId)
{
    ...
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_InputProfilePath_String, "{MVN}/input/hipmove_bindings.json");
