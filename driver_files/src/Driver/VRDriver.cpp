    ...
    if (protocol == StreamingProtocol::SPPoseQuaternion) {
        ...
        for (auto tracker_pair : trackers_) {
            ...
            if (segment_data.segmentId > -1) {
                ...
                linalg::vec <float, 4> rot(segment_data.orientation[1], segment_data.orientation[2], segment_data.orientation[3], segment_data.orientation[0]);
                linalg::vec <float, 4> rot_norm = linalg::normalize(rot);
                ...
                linalg::mat<float, 4, 4> vrMatrix = linalg::mul(linalg::mul(ZtoYupMatrix, mocapMatrix), ZtoYUpInv);
                tx = vrMatrix.w.x;
                ty = vrMatrix.w.y;
                tz = vrMatrix.w.z;

                linalg::vec<float, 4> rotQuat = linalg::rotation_quat(linalg::mat<float, 3, 3>(
                    linalg::vec<float, 3>(vrMatrix.x.x, vrMatrix.x.y, vrMatrix.x.z),
                    linalg::vec<float, 3>(vrMatrix.y.x, vrMatrix.y.y, vrMatrix.y.z),
                    linalg::vec<float, 3>(vrMatrix.z.x, vrMatrix.z.y, vrMatrix.z.z)
                ));
